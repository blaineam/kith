...
| **3. Account-state self-sync** | A per-account state blob (roster, circles, contacts, profile, settings, blocked list, read state) **self-sealed to the account's own devices** and synced via the mailbox; CRDT/LWW merge so devices converge. Gives "my devices show the same thing." | `p2pcore::selfsync` + relay channel | 🟡 **CRDT + channel done**; **iOS sync loop v1 live** (profile + settings; over a relay OR the user's S3 bucket — no relay required); contacts/blocked/circles + Android/desktop ahead |
